...
class OpenAiServiceMockTest {
    ...
    fun `generateOffer should return mock response with requested data`(): Unit = runBlocking {
        ...
        assertEquals("heat_pump", response.leadSummary.primaryProduct)
        assertTrue(response.leadSummary.buildingAssumptions.any { it.contains("1985") })
        assertTrue(response.leadSummary.buildingAssumptions.any { it.contains("gas") })
